...
public class BucKitItem: NSManagedObject {
    func loadPlaceMark() {
        if let latitude = latitude, let longitude = longitude {
            let latitudeDoub = Double(truncating: latitude)
            let longitudeDoub = Double(truncating: longitude)
            let location = CLLocation(latitude: latitudeDoub, longitude: longitudeDoub)
